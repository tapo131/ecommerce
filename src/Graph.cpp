...
void Graph::calc_node_distance1()
{
	
	for(int i = 0; i < num_node; i++){
		for(int j = i + 1; j < num_node; j++){
			distance_matrix[i][j] = distance_matrix[j][i] = distance(nodes[i].x,nodes[i].y,nodes[j].x,nodes[j].y);
		}
	}
}
double Graph:: distance(double lati1,double longti1,double lati2,double longti2) {
...
{    
	...
	this->bike_capacity = bike_capacity;
	...
	for(int i = 0; i < num_node; i++){
		...
		if(num_car!=1237){
			...
			nodes[i].x = param1[i][0];
			nodes[i].y = param1[i][1];
			//std::cout<<"P"<<" "<<nodes[i].x<<" "<<nodes[i].y<<std::endl;
		}
		...
		if(num_car!=1237)nodes[i].service_time = 90; //param[i][6];
		else nodes[i].service_time =1000;
		nodes[i].penalty_cost= 10000*nodes[i].demand;
	}
	...
	for(int i = num_bike; i < num_car; i++){
		...
		if(num_car==1237)cars[i].car_capacity = 600;
		cars[i].now_load = 0;
		cars[i].now_idx = 0;
		cars[i].car_cost= 60;
		cars[i].tour_cost = 0;
	...
	if(num_car!=1237)calc_node_distance();
	else calc_node_distance1();
}
...
void Graph::show_each_car_tour() const
{
	...
	for(int i = 0; i < num_car; i++){
		if(cars[i].tour.size() > 1){
			...
			average_fill+=(double)cars[i].now_load/cars[i].car_capacity*100;			
			...
			std::cout << ", average fill:" << average_fill;
	...
	std::cout << "average fill:" << average_fill/count << "%" << std::endl;
	std::cout << "total cost:" << 1000*total_tour_cost+total_penalty_cost << "đ" << std::endl;
	std::cout << "total penalty cost:" << total_penalty_cost << "đ" << std::endl;
}

...
const double Colony::alpha = 2.0;// prob_next_city
...
void Colony::move_ants()
{
	...
	while(!is_all_visited()){
		...
		while (feasible_edges.empty() && cars[car_idx].now_time <= close){
	...
	// 	if(idx>=num_car)break;
	// 	if(cars[idx].tour.empty()){
	// 		cars[idx].add_node(&nodes[chose_depot()], distance_matrix);// add depot
	// 		int r=rand()%posible.size();
	// 		while(cars[idx].ok_capacity(nodes[posible[r]])){
	// 			cars[idx].add_node(&nodes[posible[r]],distance_matrix);
	// 			//posible.erase(posible.begin()+r);
	// 			r=rand()%posible.size();
	// 			//std::cout<<posible[r]<<"P"<<idx<<std::endl;
	// 		}
	// 		idx++;
	// 	}
	// 	else idx++;
	// 	if(cars[idx].now_idx >= num_depot){// in case the vehicle did not return back to the depot
	// 	std::pair <double,int> near_depot[num_depot];
	// 	for(int depot=0;depot<num_depot;depot++){
	// 		near_depot[depot].first= distance_matrix[cars[idx].now_idx][depot];
	// 		near_depot[depot].second=depot;
	// 	}
	// 	sort(near_depot,near_depot+num_depot-1);
	// 	// std::cout<<"near"<<near_depot[0].first<<" "<<near_depot[0].second<<'\n';
	// 	cars[idx].add_node(&nodes[near_depot[0].second],distance_matrix);
	// 	}
	// }
}
...
Colony::Colony(std::vector<std::vector<int> > param,
				int car_capacity,int bike_capacity, int num_car,int num_bike,int num_depot,std::vector<std::vector<double> >param1) : Graph(param, car_capacity,bike_capacity, num_car,num_bike,num_depot,param1)
{
